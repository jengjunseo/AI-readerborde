...
describe("ranking method v1", () => {
  ...
  it("makes price a deterministic 3:1 input/output sort", () => { const snapshot = buildSnapshot(curatedModels, "2026-09-18"); const entry = snapshot.boards.price.entries[0]; expect(entry.model.slug).toBe("qwen3-235b"); expect(entry.value).toBe(blendedPrice(curatedModels.find((model) => model.slug === "qwen3-235b")!)); });
