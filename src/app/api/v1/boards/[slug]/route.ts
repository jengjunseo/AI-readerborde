...
export async function GET(_: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const entries = await db.select({ rank: rankingEntries.rank, value: rankingEntries.value, previousRank: rankingEntries.previousRank, isNew: rankingEntries.isNew, model: models.name, modelSlug: models.slug, provider: providers.name, version: modelVersions.version, breakdown: boardScores.breakdown, coverage: boardScores.coverage }).from(rankingEntries).innerJoin(modelVersions, eq(rankingEntries.modelVersionId, modelVersions.id)).innerJoin(models, eq(modelVersions.modelId, models.id)).innerJoin(providers, eq(models.providerId, providers.id)).innerJoin(boardScores, and(eq(boardScores.pipelineRunId, snapshot.pipelineRunId), eq(boardScores.modelVersionId, modelVersions.id), eq(boardScores.boardSlug, slug))).where(eq(rankingEntries.snapshotId, snapshot.id));
